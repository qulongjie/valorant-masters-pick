...
function App() {
  ...
  const handleSubmitPrediction = (
    ...
    // Refresh predictions state
    const newPredictions = { ...predictions, [matchId]: newPrediction };
    setPredictions(newPredictions);

    // Reward points for predicting (+3 mock points)
    const match = MATCHES.find(m => m.id === matchId);
    const teamA = TEAMS.find(t => t.id === match?.teamAId);
    const teamB = TEAMS.find(t => t.id === match?.teamBId);
    const description = `预测了 ${teamA?.name} vs ${teamB?.name} 比分`;
    
    addPointHistory(description, 3);
    ...
    switch (activeTab) {
      ...
      case 'result':
        if (!match || !teamA || !teamB || !pred) {
          return (
            <div className="py-20 text-center text-xs text-grey-secondary select-none">
              未选中比赛结果数据，请返回赛程页面进行预测。
            </div>
          );
        }
        return (
          <ResultPage
            ...
            teamB={teamB}
            prediction={pred}
            onGenerateShare={() => setActiveTab('share')}
  ...
  const handleTabChange = (tab: TabType) => {
    // Reset temporary states when clicking bottom nav
    if (tab === 'profile') {
      // Clear result match id if moving to profile directly
  ...
  return (
    <>
      ...
      <div 
        className={`fixed top-8 left-1/2 -translate-x-1/2 z-[100] w-[90%] max-w-[340px] transition-all duration-300 transform pointer-events-none select-none ${
      ...
      {/* Render fixed bottom navigation bar ONLY if not in share page */}
      {activeTab !== 'share' && (
        <BottomNav 
          activeTab={activeTab === 'result' ? 'predict' : activeTab as TabType} 
